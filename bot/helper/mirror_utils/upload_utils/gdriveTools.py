...
class GoogleDriveHelper:
        ...
        for content in self.telegraph_content:
            if nxt_page == 1:
                ...
                if prev_page <= self.num_of_path:
                    content += (
                        f'<b><a href="https://telegra.ph/{self.path[prev_page]}">Prev</a></b>'
                    )
                    prev_page += 1
                if nxt_page < self.num_of_path:
                    content += (
                        f'<b> | <a href="https://telegra.ph/{self.path[nxt_page]}">Next</a></b>'
                    )
                    nxt_page += 1
    ...
    def count(self, link):
        ...
        try:
            drive_file = (
                self.__service.files()
                .get(fileId=file_id, fields="id, name, mimeType, size", supportsTeamDrives=True)
                .execute()
            )
