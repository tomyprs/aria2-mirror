...
def start(update, context):
    ...
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        if update.message.chat.type == "private":
            ...
            sendMessage(
                f"Hey <b>{update.effective_user.first_name}</b>. This is a bot which can mirror all your links to Google drive!\nType /help to see all the commands",
                context.bot,
                update,
            )
    else:
        sendMessage(
            f"Hey, <b>{update.message.chat.first_name}.</b> You're not authorized to use this bot.",
            context.bot,
            update,
        )
...
def main():
    ...
    ping_handler = CommandHandler(
        BotCommands.PingCommand, ping, filters=CustomFilters.owner_filter | CustomFilters.sudo_user
    )
