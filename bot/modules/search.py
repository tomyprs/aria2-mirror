    ...
    async with search_lock:
        ...
        if (time.time() - get_time) > 3600:
            ...
            async with session.get(
                f'https://{"sukebei." if sukebei else ""}nyaa.si/?page=rss&q={urlencode(query)}'
            ) as resp:
...
async def init_search(client, message, query, sukebei):
    ...
    if not result:
        ...
        if pages == 1:
            buttons.pop()
        reply = await message.reply_text(result, reply_markup=InlineKeyboardMarkup([buttons]))
...
async def nyaa_callback(client, callback_query):
    ...
    async with callback_lock:
        ...
        buttons = [
            InlineKeyboardButton("Back", "nyaa_back"),
