...
def _watch(bot: Bot, update, isTar=False):
    ...
    try:
        ...
        msg += (
            "This file will be downloaded in 720p quality and it's name will be <b>My video bro</b>"
        )
    ...
    threading.Thread(
        target=ydl.add_download, args=(link, f"{DOWNLOAD_DIR}{listener.uid}", qual, name)
    ).start()
